...
function createEmail(to: string, subject: string, htmlContent: string): string {
  const email = [
    `From: Vincci Pub <${GMAIL_USER_EMAIL}>`,
    `To: ${to}`,
...
serve(async (req) => {
  ...
  try {
    ...
    // Email para o estabelecimento (admin)
    ...
    // Send admin email
    const adminEmailResponse = await sendGmailEmail(
      GMAIL_USER_EMAIL!,
      `🎉 Novo Pedido Confirmado - ${pixId}`,
      adminEmailHTML
    );
    ...
    try {
      customerEmailResponse = await sendGmailEmail(
        customerEmail,
        `✅ Pedido Confirmado - Vincci Pub`,
        customerEmailHTML
      );
